...
double time_collectiveExperience = 0.0;
...
void collectiveExperience() {
    ...
    #pragma omp parallel for
    for(int i = 0; i < NUM_FISH; i++) {
        double localNumerator = school[i].currentObjective * school[i].weight;
        double localDenominator = school[i].currentObjective;

        #pragma omp critical
        {
            barycentreNumerator += localNumerator;
            barycentreDenominator += localDenominator;
        }
    }

    double endTime = omp_get_wtime();
    time_collectiveExperience = endTime - startTime;  // Assuming you've declared this variable globally or somewhere in scope
...
int main() {
    printf("Num_Threads\ttotalTime\teat()Time\tcollectiveExperience()Time\n");
    for (int num_threads = 1; num_threads <= 16; num_threads++) {
        ...
        initializeFish();
        optimization();
        printf("%.1lf\n", time_reduction); //eat() time
        printf("%.1lf seconds\n",  time_collectiveExperience); //Time taken by reduction in collectiveExperience():
        freeAll();
    }
    return 0;
}
